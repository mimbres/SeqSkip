...
def validate():
    ...
    for val_session in trange(len(val_sessions_iter), desc='val-sessions', position=2):
        ...
        sim_score = torch.FloatTensor(np.zeros((batch_sz, 10, 10 ,2))).detach().cpu() # bx8x7*2 (b x que x sup x class)
        sim_score[:,:,:,0] = (y_hat_relation.detach().cpu()*(y_sup_ext==0).float() + (1-y_hat_relation.detach().cpu())*(y_sup_ext==1).float()).view(-1,10,10)
        sim_score[:,:,:,1] = (y_hat_relation.detach().cpu()*(y_sup_ext==1).float() + (1-y_hat_relation.detach().cpu())*(y_sup_ext==0).float()).view(-1,10,10)
        sim_score = sim_score * y_mask.detach().cpu().repeat(1,1,1,2)
        y_pred = torch.argmax(sim_score.sum(2),2).numpy()
        ...
        if (val_session+1)%10000 == 0:
            tqdm.write(np.array2string(sim_score[0,:,:,1]))
