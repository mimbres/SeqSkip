...
parser.add_argument("-s","--save_path",type = str, default = "./save/exp_rnb1_2048_nolog/")
...
def validate():
    ...
    for val_session in trange(len(val_sessions_iter), desc='val-sessions', position=2):
        ...
        x_log_sup = Variable(torch.FloatTensor([])).cuda(GPU)
        x_log_que = Variable(torch.FloatTensor([])).cuda(GPU)
