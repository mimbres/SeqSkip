...
parser.add_argument("-s","--save_path",type=str, default="./save/exp_MH_seq1H_v4_dt2_TRF/")
...
class SeqModel(nn.Module):
    def __init__(self, input_dim=INPUT_DIM, e_ch=128, d_ch=128, use_glu=USE_GLU):
        ...
        self.feature_sup = nn.Sequential(nn.Conv1d(input_dim,e_ch,1), nn.ReLU(),
                                        nn.Conv1d(e_ch,e_ch,1))
        self.feature_que = nn.Sequential(nn.Conv1d(input_dim,e_ch,1), nn.ReLU(),
                                        nn.Conv1d(e_ch,e_ch,1))
        
        self.mh_sup = MultiHeadAttention(query_dim=e_ch ,key_dim=e_ch, num_units=e_ch, dropout_p=0.7, h=8)
        self.mh_que = MultiHeadAttention(query_dim=e_ch ,key_dim=e_ch, num_units=e_ch, dropout_p=0.7, h=8)
        self.mh_rel = MultiHeadAttention(query_dim=e_ch ,key_dim=e_ch, num_units=e_ch, dropout_p=0.7, h=8)
        
        self.mh_tot = MultiHeadAttention(query_dim=256 ,key_dim=256, num_units=256, dropout_p=0.95, h=8)
        
        
#        self.enc = SeqEncoder(input_ch=256, e_ch=e_ch,
#                                  h_k_szs=[2,2,2,3,1,1], #h_k_szs=[2,2,2,3,1,1],
#                                  h_dils=[1,2,4,8,1,1], #h_dils=[1,2,4,8,1,1],
#                                  use_glu=use_glu) # bx128*10
        
        
        self.mh_tot2 = MultiHeadAttention(query_dim=256 ,key_dim=256, num_units=256, dropout_p=0.95, h=8)
        self.mh_tot3 = MultiHeadAttention(query_dim=256 ,key_dim=256, num_units=256, dropout_p=0.95, h=8)
        self.mh_tot4 = MultiHeadAttention(query_dim=256 ,key_dim=256, num_units=256, dropout_p=0.95, h=8)
#        self.enc2 = SeqEncoder(input_ch=256, e_ch=64,
#                          h_k_szs=[2,2,2,3,1,1], #h_k_szs=[2,2,2,3,1,1],
#                          h_dils=[1,2,4,8,1,1], #h_dils=[1,2,4,8,1,1],
#                          use_glu=use_glu) # bx128*10
        
        
        self.feature = nn.Sequential(nn.Conv1d(256,256,1), nn.ReLU(),
                                        nn.Conv1d(256,64,1), nn.ReLU())
        self.classifier = nn.Conv1d(64,1,1)
        
    def forward(self, x_sup, x_que):
        feat_sup = self.feature_sup(x_sup)
        feat_que = self.feature_sup(x_que)
        feat_sup, _ = self.mh_sup(query=feat_sup.permute(0,2,1), keys=feat_sup.permute(0,2,1))
        feat_que, _ = self.mh_que(query=feat_que.permute(0,2,1), keys=feat_que.permute(0,2,1))
        feat_rel_sup, _ = self.mh_rel(query=feat_sup, keys=feat_que)
        feat_rel_que, _ = self.mh_rel(query=feat_que, keys=feat_sup)
        
        #x, att = self.mh(query=feat_que.permute(0,2,1), keys=feat_sup.permute(0,2,1))
        x = torch.cat((feat_sup.permute(0,2,1), feat_que.permute(0,2,1)), 2)
        x_rel = torch.cat((feat_rel_sup.permute(0,2,1), feat_rel_que.permute(0,2,1)), 2)
        x = torch.cat((x, x_rel), 1)
        x, _ = self.mh_tot(query=x.permute(0,2,1), keys=x.permute(0,2,1))
        #x = self.enc(x.permute(0,2,1)) # bx128*10 
        x, _ = self.mh_tot2(query=x, keys=x)
        x, _ = self.mh_tot3(query=x, keys=x)
        x, _ = self.mh_tot4(query=x, keys=x)
        #x = self.enc2(x.permute(0,2,1))
        x = self.feature(x.permute(0,2,1))
