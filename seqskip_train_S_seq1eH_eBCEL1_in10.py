...
parser.add_argument("-s","--save_path",type=str, default="./save/exp_S_seq1eH_eBCEL1_in10/")
...
def main():  
    ...
    for epoch in trange(START_EPOCH, EPOCHS, desc='epochs', position=0, ascii=True):
        ...
        for session in trange(len(tr_sessions_iter), desc='sessions', position=1, ascii=True):
            ...
            # target mask
            target_mask = y_mask.clone().unsqueeze(1).repeat(1,128,1).cuda(GPU)
            ...
            loss1 = F.binary_cross_entropy_with_logits(input=y_hat_enc*target_mask_que, target=torch.sigmoid(enc_target)*target_mask_que)
            loss2 = F.l1_loss(input=y_hat_enc*target_mask_que, target=enc_target*target_mask_que)
            loss = loss1+loss2
