...
parser.add_argument("-s","--save_path",type = str, default = "./save/exp4/")
...
# Init neural net
FeatEnc = MLP(input_sz=29, hidden_sz=2048, output_sz=512).apply(weights_init).cuda(GPU)
RN      = RelationNetwork(input_sz=1068).apply(weights_init).cuda(GPU)
...
for epoch in trange(START_EPOCH, EPOCHS, desc='epochs', position=0):
    ...
    for session in trange(len(sessions_iter), desc='sessions', position=1):
        ...
        # Calcultate MSE loss
        #loss = F.mse_loss(input=y_hat_relation, target=y_relation)
        loss = F.binary_cross_entropy(input=y_hat_relation, target=y_relation)
